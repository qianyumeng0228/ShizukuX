...
ShizukuX 是基于 Shizuku 的增强版本，保持对现有 Shizuku 应用的兼容，同时加入更可靠的连接流程、Watchdog、Plus API、Root 兼容桥、Dhizuku 模式等能力。
...
- [权限说明](permissions.md)：ShizukuX 请求的权限、用途，以及哪些能力依赖这些权限。
- [自动化应用控制 ShizukuX](automation-apps.md)：Tasker、MacroDroid 等 Locale 兼容应用如何启动/停止服务。
...
请优先阅读 [知识库与故障排查](knowledgebase.md)。如果问题仍然存在，请到本项目的 [Issues](https://github.com/qianyumeng0228/ShizukuX/issues) 提交反馈，或使用应用内“报告错误”功能发送邮件到 `support@xiaoyuanqiang.xyz`。
