...
2. **Hidden Packages feature is unclear**: I updated the settings description to make this much clearer. If you are using both OG Shizuku and ShizukuX, you should add BOTH (\`moe.shizuku.privileged.api\` and \`af.shizuku.manager\`) to the list to prevent conflicts and ensure apps can't detect either of them.
...
gh issue comment 264 --body "To clear up the confusion about the \"two ShizukuX apps\" in the app drawer: 

Our \`AndroidManifest.xml\` accidentally had a duplicate \`LAUNCHER\` intent filter. We had one for the main app activity, and a second one for an internal shortcut (\`activity-alias\`). Because both declared the \`LAUNCHER\` category, Android OS was placing two identical ShizukuX icons in the app drawer!
...
You no longer need to factory reset to escape Device Owner mode. Once ShizukuX is set as Device Owner, a \"Clear Device Owner\" option will automatically become visible under \`Settings -> Core Framework -> Device Owner Tools\`. Tapping it will cleanly relinquish the Device Owner privileges.
